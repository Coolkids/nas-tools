            ...
            with ThreadPoolExecutor(max_workers=5) as pool:
                ...
                if media.type == MediaType.MOVIE:
                    ...
                    if scraper_movie_nfo.get("basic") or scraper_movie_nfo.get("credits"):
                        ...
                        if not os.path.exists(os.path.join(dir_path, "movie.nfo")) \
                            ...
                            tasks.append(pool.submit(
                                ...
                                media.tmdb_info,doubaninfo,scraper_movie_nfo,dir_path,file_name
                            ))
                    # poster
                    if scraper_movie_pic.get("poster"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.get_poster_image(original=True),
                            dir_path,
                            "poster"
                        ))
                    # backdrop
                    if scraper_movie_pic.get("backdrop"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.get_backdrop_image(default=False, original=True),
                            dir_path,
                            "fanart"
                        ))
                    # background
                    if scraper_movie_pic.get("background"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_background(media_type=media.type, queryid=media.tmdb_id),
                            dir_path,
                            "background"
                        ))
                    # logo
                    if scraper_movie_pic.get("logo"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_logo(media_type=media.type, queryid=media.tmdb_id),
                            dir_path,
                            "logo"
                        ))
                    # disc
                    if scraper_movie_pic.get("disc"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_disc(media_type=media.type, queryid=media.tmdb_id),
                            dir_path,
                            "disc"
                        ))
                    # banner
                    if scraper_movie_pic.get("banner"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_banner(media_type=media.type, queryid=media.tmdb_id),
                            dir_path,
                            "banner"
                        ))
                    # thumb
                    if scraper_movie_pic.get("thumb"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_thumb(media_type=media.type, queryid=media.tmdb_id),
                            dir_path,
                            "thumb"
                        ))
                # 电视剧
                    ...
                    if not os.path.exists(os.path.join(os.path.dirname(dir_path), "tvshow.nfo")):
                        if scraper_tv_nfo.get("basic") or scraper_tv_nfo.get("credits"):
                            ...
                            # 根目录描述文件
                            tasks.append(pool.submit(
                                self.gen_tv_nfo_file,
                                media.tmdb_info, doubaninfo, scraper_tv_nfo, os.path.dirname(dir_path)
                            ))
                    # poster
                    ...
                    if scraper_tv_pic.get("background"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_background(media_type=media.type, queryid=media.tvdb_id),
                            dir_path,
                            "show"
                        ))
                    # logo
                    if scraper_tv_pic.get("logo"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_logo(media_type=media.type, queryid=media.tvdb_id),
                            dir_path,
                            "logo"
                        ))
                    # clearart
                    if scraper_tv_pic.get("clearart"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_disc(media_type=media.type, queryid=media.tvdb_id),
                            dir_path,
                            "clearart"
                        ))
                    # banner
                    if scraper_tv_pic.get("banner"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_banner(media_type=media.type, queryid=media.tvdb_id),
                            dir_path,
                            "banner"
                        ))
                    # thumb
                    if scraper_tv_pic.get("thumb"):
                        tasks.append(pool.submit(
                            self.__save_image,
                            media.fanart.get_thumb(media_type=media.type, queryid=media.tvdb_id),
                            dir_path,
                            "thumb"
                        ))
                    ...
                    if scraper_tv_nfo.get("season_basic"):
                        if not os.path.exists(os.path.join(dir_path, "season.nfo")):
                            ...
                            tasks.append(pool.submit(season_basic_task))
                    ...
                    if scraper_tv_nfo.get("episode_basic") \
                        ...
                        if not os.path.exists(os.path.join(dir_path, "%s.nfo" % file_name)):
                            ...
                            tasks.append(pool.submit(episode_basic_task))

                    # season poster
                    if scraper_tv_pic.get("season_poster"):
                        def season_poster_task():
                            ...
                            if seasonposter:
                                ...
                                season_info = self.media.get_tmdb_tv_season_detail(tmdbid=media.tmdb_id,
                                                                                  season=int(media.get_season_seq()))
                                if season_info:
                                    self.__save_image(TMDB_IMAGE_W500_URL % season_info.get("poster_path"),
                                                      os.path.dirname(dir_path),
                                                      season_poster)

                        tasks.append(pool.submit(season_poster_task))



                    # season banner
                    if scraper_tv_pic.get("season_banner"):
                        def season_banner_task():
                        ...
                        tasks.append(pool.submit(season_banner_task))

                    # season thumb
                    if scraper_tv_pic.get("season_thumb"):
                        def season_thumb_task():
                        ...
                        tasks.append(pool.submit(season_thumb_task))

                    # episode thumb
                    if scraper_tv_pic.get("episode_thumb"):
                        def episode_thumb_task():
                        ...
                        tasks.append(pool.submit(episode_thumb_task))

                for f in as_completed(tasks):
                    try:
                        f.result()
                    except Exception as e1:
                        log.error(f"【Scraper】刮削元数据子任务发生异常", e1)

        except Exception as e:
            ExceptionUtils.exception_traceback(e)
            log.error(f"【Scraper】刮削元数据发生异常", e)
